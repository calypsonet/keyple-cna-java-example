...
class MainActivity :
  ...
  private fun handleStorageCard(storageCard: StorageCard) {
    ...
    val duration = measureTimeMillis {
      // MIFARE Classic requires sector authentication before read/write
      ...
      // Determine safe block range based on card type
      var startBlock = 0
      var endBlock = storageCard.productType.blockCount - 1

      if (storageCard.productType == ProductType.MIFARE_CLASSIC_1K) {
        startBlock = 4 // Start at first block of sector 1
        // IMPORTANT: Stop at block 6. Block 7 is the Sector Trailer (Keys + Access Bits).
